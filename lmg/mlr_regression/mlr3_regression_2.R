...
for (i in unique(idsml[which(!(idsml %in% idstrain))])){
  ...
  preddf[preddf$ids == i,]$pctdiffstr_truth <- ml$percent.of.different.structure[prediction$data$row_ids[which(idsml==i)][which.min(prediction$data$truth[which(idsml==i)])]]
...
pddf2 <- data.frame(pcdiff=preddf$pctdiffstr_truth, set = "True best algorithms")
pddf3 <- data.frame(pcdiff=preddf$pctdiffstr_pre, set = "Predicted algorithms")

pddfplot <- rbind(pddf,pddf2,pddf3) 

my_comparisons <- list( c(2, 3), c(1, 2), c(1, 3) )

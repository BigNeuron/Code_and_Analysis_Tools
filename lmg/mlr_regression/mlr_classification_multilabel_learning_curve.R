...
groupsdf$algorithm[groupsdf$algorithm=="app2"] <- NULL
groupsdf$algorithm[groupsdf$algorithm=="app2new1"] <- NULL
groupsdf$algorithm[groupsdf$algorithm=="app2new2"] <- NULL
groupsdf$algorithm[groupsdf$algorithm=="app2new3"] <- "app2"
groupsdf$algorithm[groupsdf$algorithm=="Advantra"] <- NULL
groupsdf$algorithm[groupsdf$algorithm=="Advantra_updated"] <- "Advantra"
groupsdf$algorithm[groupsdf$algorithm=="neutube"] <- NULL
groupsdf$algorithm[groupsdf$algorithm=="neutube_updated"] <- "neutube"
groupsdf$algorithm[groupsdf$algorithm=="pyzh"] <- NULL
groupsdf$algorithm[groupsdf$algorithm=="pyzh_updated"] <- "pyzh"
groupsdf$algorithm[groupsdf$algorithm=="LCMboost"] <- NULL
groupsdf$algorithm[groupsdf$algorithm=="LCMboost_updated"] <- NULL
groupsdf$algorithm[groupsdf$algorithm=="LCMboost_3"] <- "LCMboost"
groupsdf$algorithm[groupsdf$algorithm=="fastmarching_spanningtree"] <- NULL
groupsdf$algorithm[groupsdf$algorithm=="fastmarching_spanningtree_updated"] <- "fastmarching_spanningtree"
groupsdf$algorithm[groupsdf$algorithm=="axis_analyzer"] <- NULL
groupsdf$algorithm[groupsdf$algorithm=="axis_analyzer_updated"] <- "axis_analyzer"
groupsdf$algorithm[groupsdf$algorithm=="NeuronChaser"] <- NULL
groupsdf$algorithm[groupsdf$algorithm=="NeuronChaser_updated"] <- "NeuronChaser"
groupsdf$algorithm[groupsdf$algorithm=="meanshift"] <- NULL
groupsdf$algorithm[groupsdf$algorithm=="meanshift_updated"] <- "meanshift"
groupsdf$algorithm[groupsdf$algorithm=="NeuroGPSTree"] <- NULL
groupsdf$algorithm[groupsdf$algorithm=="NeuroGPSTree_updated"] <- "NeuroGPSTree"
groupsdf$algorithm[groupsdf$algorithm=="ENT"] <- NULL
...
pcdiffs <- ml$percent.of.different.structure
...
# ggplotly(p)
pddf <- data.frame(pcdiff=ml$percent.of.different.structure, set = "All algorithms")
...
lrn = setHyperPars(makeLearner("classif.rpart"), 
                   ...
                   minsplit = 2, 
                   maxdepth = 30)
# lrn = makePreprocWrapperCaret(lrn, ppc.BoxCox = TRUE)
# lrn = setHyperPars(makeLearner("classif.svm"), kernel = "polynomial",
#                    degree = 1, cost = 2.15, gamma = 9.07)
...
dt_task <- makeClassifTask(data=training, target="bestalg", blocking=as.factor(ids_train))
# dt_task <- makeClassifTask(data=training, target="bestalg")
dt_prob <- makeLearner('classif.svm', predict.type="prob")

dt_prob = setHyperPars(makeLearner("classif.svm", predict.type="prob"), kernel = "polynomial",
                   degree = 1, cost = 2.15, gamma = 9.07)

fvalues <- generateFilterValuesData(dt_task, method = c("FSelector_information.gain","FSelector_chi.squared", "FSelector_gain.ratio")) 
...
preddf <- data.frame(ids=test_pred$data$id)
preddf$pred <- ""
preddf$truth <- ""
preddf$pctdiffstr_pred <- 0
preddf$pctdiffstr_truth <- 0 
for (i in test_pred$data$id){
  # preddf[preddf$ids == i,]$truth <- bestalgs[prediction$data$row_ids[which(idsml==i)][which.min(prediction$data$response[which(idsml==i)])]]
  # preddf[preddf$ids == i,]$pred <- algs[prediction$data$row_ids[which(idsml==i)][which.min(prediction$data$response[which(idsml==i)])]]
  # preddf[preddf$ids == i,]$pctdiffstr_truth <- ml$percent.of.different.structure[prediction$data$row_ids[which(idsml==i)][which.min(prediction$data$response[which(idsml==i)])]]
  # preddf[preddf$ids == i,]$pctdiffstr_pred <- ml$percent.of.different.structure[prediction$data$row_ids[which(idsml==i)][which.min(prediction$data$response[which(idsml==i)])]]
  preddf[preddf$ids == i,]$pctdiffstr_pred <- min(pcdiffs[idsml==idsml[i] & algs == test_pred$data$response[test_pred$data$id == i]])
}
plot(density(preddf$pctdiffstr_pred),main="Percentage of difference to GS for predicted algorithm")
pddf3 <- data.frame(pcdiff=preddf$pctdiffstr_pre, set = "Predicted algorithms")

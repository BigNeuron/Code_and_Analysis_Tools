...
dir.create("gold_163_all_soma_sort_s1_isotropic")
dir.create("0401_gold163_all_soma_sort_isotropic")

# psdf <- psdf[psdf$id==99,]

for(i in psdf$id){
  goldfn <- list.files(paste0('gold_163_all_soma_sort_s1_restim/',i),pattern='/*_resampled.swc.radius.swc$')
  consfn <- list.files(paste0('0401_gold163_all_soma_sort_restim/',i),pattern='/*radius.swc$')
  ...
  if(length(goldfn)>0){
    ingoldfn <- paste0('gold_163_all_soma_sort_s1_restim/',i,'/',goldfn)
    outgoldfn <- paste0('gold_163_all_soma_sort_s1_isotropic/',i,'/',goldfn)
  }
  
  if(length(consfn)>0){
    inconsfn <- paste0('0401_gold163_all_soma_sort_restim/',i,'/',consfn)
    outconsfn <- paste0('0401_gold163_all_soma_sort_isotropic/',i,'/',consfn)
  }
  
  # if(length(autofn)>0){
  #   inautofn <- paste0('gold_163_all_soma_sort_s1_restim/',i,'/auto_recons/',autofn)
  #   outautofn <- paste0('gold_163_all_soma_sort_s1_isotropic/',i,'/auto_recons/',autofn)
  # }
  # 
  # if(length(procfn)>0){
  #   inprocfn <- paste0('gold_163_all_soma_sort_s1_restim/',i,'/processed/',procfn)
  #   outprocfn <- paste0('gold_163_all_soma_sort_s1_isotropic/',i,'/processed/',procfn)
  # }
  
  dir.create(paste0('gold_163_all_soma_sort_s1_isotropic/',i))
  # dir.create(paste0('gold_163_all_soma_sort_s1_isotropic/',i,'/processed'))
  # dir.create(paste0('gold_163_all_soma_sort_s1_isotropic/',i,'/auto_recons'))
  dir.create(paste0('0401_gold163_all_soma_sort_isotropic/',i))
  ...
  foreach(j = seq_along(ingoldfn)) %dopar% {
    ...
    infile$V3 <- infile$V3*xy
    infile$V4 <- infile$V4*xy
    infile$V5 <- infile$V5*xy
    infile$V6 <- infile$V6*xy
